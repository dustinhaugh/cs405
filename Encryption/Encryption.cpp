...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
    ...
    {   // TODO: student need to change the next line from output[i] = source[i]
        ...
        // Loop through the string until string_length
        for (size_t i = 0; i < source_length; ++i)
        {
            // Transform each character based on an XOR of the key, source: https://www.101computing.net/xor-encryption-algorithm/ 
...
std::string read_file(const std::string& filename)
{
    ...
    if (file_stream.is_open()) {
        // Read the file content line by line and append it to file_text
        std::string line;
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    //  file format goes as follows:
    //  Line 1: student name
    file_stream << student_name << "\n";
